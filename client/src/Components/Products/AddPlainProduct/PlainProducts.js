...
const PlainProducts = () => {
 ...
 useEffect(() => {
  const fetchPlainLot = async () => {
    try {
      // new merged endpoint: GET /api/v1/lot/:id
      const res = await axios.get(`${REACT_APP_BACKEND_SERVER_URL}/api/v1/lot/${lot_id}`);
      // New controller returns { msg, products, success } where `products` is an array
      // Each element is a lot_info record and may include plainProduct or products depending on type
      const payload = res.data || {};
      const lotsArray = payload.products || payload.result || [];
      const lotEntry = Array.isArray(lotsArray) && lotsArray.length ? lotsArray[0] : null;

      // If this is PLAIN lot, backend includes `plainProduct` relation
      const plainProductsFromApi = lotEntry?.plainProduct || lotEntry?.plainProducts || [];

      setProducts(Array.isArray(plainProductsFromApi) ? plainProductsFromApi : []);
      // set display label: lot_name is the new visible id
      if (lotEntry?.lot_name) setLotNumber(lotEntry.lot_name);
      // fallback: if backend still returns old shape
...
const handleBulkExportPdf = async (items) => {
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      const res = await axios.delete(`${REACT_APP_BACKEND_SERVER_URL}/api/v1/plainProducts/${productId}`);
      if (res.status === 200) {
...
const handleSavePlainProduct = async () => {
  ...
  if (!fileBlobRef.current) {
    toast.error("Please capture an image (camera)");
    return;
  }
  // prepare formdata
  const fd = new FormData();

  // --- VERY IMPORTANT: send multiple lot keys to be robust
  // backend controller sometimes expects `lotId`, other times `lot_id` or `plainLotId`.
  // we send them all (server will pick whichever it reads).
  fd.append("lotId", String(lot_id));      // controller often reads this
  fd.append("lot_id", String(lot_id));     // prisma field style
  fd.append("plainLotId", String(lot_id)); // previous client field (safe)

  console.log("Submitting lot identifiers:", { lot_id });

  fd.append("productName", productName);
  fd.append("workerName", workerName);
  fd.append(
    "goldSmithCode",
    (goldsmithsList.find((gs) => String(gs.id) === String(selectedGoldsmithId))?.goldSmithCode) || ""
  );
  fd.append(
    "itemCode",
    (itemsList.find((it) => String(it.id) === String(selectedItemId))?.itemCode) || ""
  );
  fd.append("grossWeight", String(grossWeight));
  fd.append("stoneWeight", String(stoneWeight));
  fd.append("netWeight", String(netWeight));
  fd.append("file", fileBlobRef.current);

  try {
    const res = await axios.post(`${REACT_APP_BACKEND_SERVER_URL}/api/v1/plainProducts`, fd, {
      headers: { "Content-Type": "multipart/form-data" },
      timeout: 30000,
    });

    // backend shape may vary — try several options
    const created =
      res.data.createdProduct ||
      res.data.productInfo ||
      (res.data.productInfo && res.data.productInfo.createdProduct) ||
      res.data.newProduct ||
      res.data;

    // Normalize new product object (best-effort)
    let newP;
  ...
  try {
    const res = await axios.put(
      `${REACT_APP_BACKEND_SERVER_URL}/api/v1/plainProducts/${editProduct.id}`,
      {
        productName: editProduct.productName,
        workerName: editProduct.workerName,
        grossWeight: editProduct.grossWeight,
        stoneWeight: editProduct.stoneWeight,
        netWeight: editProduct.netWeight
      }
    );
    ...
    setProducts((prev) =>
      prev.map((p) => (p.id === editProduct.id ? res.data.updatedProduct : p))
    );
  ...
  return (
    <>
              ...
              <tbody>
                {filterProducts.map((product, index) => (
                  <tr key={product.id || index}>
                    <td>{index + 1}</td>
                    <td>
                      <input value={product.product_id || product.id || ""} readOnly
                    ...
                    <td>
                      <input style={{ fontSize: "0.95rem" }} value={product.status ? "UnSold" : "Sold" || ""}  readOnly />
                    </td>
                    <td>
      ...
      {showPreviewModal && previewUrl && (
        <div className="modal-overlay" style={modalOverlayStyle}>
          <div className="modal-box" style={{ ...modalBoxStyle, width: "min(90vw, 720px)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
