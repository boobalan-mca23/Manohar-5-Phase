...
// const ws = new WebSocket("wss://wss1_ibacus.octosignals.com/ws/stream");
...
export const handleWeight = () => {
  return new Promise((resolve, reject) => {
    try {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data);
        console.log("WS DATA:", data.weight);
        resolve(data.weight);   // return weight here
        ws.close();             // close after receiving
      };

      ws.onerror = () => {
        reject("Weight machine not connected");
      };
...
// The WebSocket connection and handleWeight function were moved to client/src/services/webSocket.js
// Components needing real-time weight data should use the subscribeWeight function from that service.
// Example: import { subscribeWeight } from '../../services/webSocket';
